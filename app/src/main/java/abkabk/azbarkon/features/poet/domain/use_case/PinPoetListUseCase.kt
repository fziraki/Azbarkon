...
class PinPoetListUseCase @Inject constructor(
    private val pinnedPoetRepository: PinnedPoetRepository
) {
    operator fun invoke(poetsIds: List<Int>): Flow<Resource<List<Long>>> {
        return pinnedPoetRepository.insertPinnedPoets(poetsIds)
    }
}